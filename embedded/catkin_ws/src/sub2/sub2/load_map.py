...
class loadMap(Node):
    ...
    def __init__(self):
        ...
        pkg_path =os.getcwd()
        full_path="C:/Users/SSAFY/Desktop/catkin_ws/src/sub2/map/map4.txt"
        print(full_path)
        self.f=open(full_path,'r')              # 읽기 모드로 파일을 열기

        line=self.f.readlines()
        line_data=line[0].split()

        for num,data in enumerate(line_data) :  # 인덱스와 값
            self.map_data[num]=int(data)


        map_to_grid=np.array(self.map_data)     # 1차원 배열
        grid=np.reshape(map_to_grid,(350, 350)) # 2차원 배열로 재구성



        for y in range(350):
            for x in range(350):
                if grid[x][y] == 100 : # 해당 셀이 점유 영역이라면

                    '''
                    로직 3. 점유영역 근처 필터처리
                    '''
                    for box_x in range(-6, 7):
                        for box_y in range(-6, 7):
                            if  0< x+box_x < 350 and 0 < y+box_y <350 and grid[x+box_x][y+box_y]<80 :
                                grid[x+box_x][y+box_y]=127
                                # 127 은 접근이 제한된 영역

                # if grid[x][y] == 127 : # 해당 셀이 점유 영역이라면
        ...
        # 다른 노드에서 이 데이터를 구독할 수 있도록 1차원 배열로 만든 후 리스트로 변환함
